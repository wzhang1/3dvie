...
def load_batch(imgs, batch_head, batch_sz, size_x, size_y, cnt, test_folder):
	...
	img_name = imgs[index]
        ...
        resize_x = size_x *6 / 5
        resize_y = size_y * 6 / 5
        bouder_x = (resize_x - size_x) / 2
        bouder_y = (resize_y - size_y) / 2
        #print(bouder_x, bouder_y)
        img= cv2.resize(img, (resize_x, resize_y), interpolation=cv2.INTER_CUBIC)
        #print(img.shape)
        crop_img = img[bouder_y:bouder_y + size_y, bouder_x:bouder_x + size_x ]
        #print(crop_img.shape)
        
        img = np.float32(crop_img)
...
im_idxs12, preds12, im_probs12 = predict("../model/3dvie-resnet-18", 9, 0, "../data/image_dev.lst", 540, 540, 128)
print(im_idxs12)
print(len(preds12))
...
with open("3classes"+".csv" , 'w') as opfile:
