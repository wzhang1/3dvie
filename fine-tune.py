...
def get_fine_tune_model(sym, arg_params, num_classes, layer_name):
    ...
    net = mx.symbol.FullyConnected(data=net, num_hidden=num_classes, name='fc', lr_mult=2)
    net = mx.symbol.SoftmaxOutput(data=net, name='softmax')
    new_args = dict({k:arg_params[k] for k in arg_params if 'fc' not in k})
    return (net, new_args)

def load_pre_model(epoch):
    prefix1 = './model_level3/3dvie-resnet-18bn32'
    sym, arg_params, aux_params = mx.model.load_checkpoint(prefix1, epoch)
...
if __name__ == "__main__":
    ...
    parser.set_defaults(image_shape='3,320,320', num_epochs=30,
                        lr=.0001, lr_step_epochs='15, 20, 28', wd=0, mom=0.1, top_k = 2)
    ...
    if load_pre:
        epoch = 12
        print("loadling old epoch")
        (new_sym, new_args) = load_pre_model(epoch)
    else:
